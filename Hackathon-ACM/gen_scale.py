...
# entry_time root_not  Octave  Width  Num Notes Dec
    ...
    #Also returns that arpegio cause it can I guess
        x = self.note_list
        sublist = [x[0],x[2],x[4],x[6],x[4],x[2],x[0]]
    ...
    def mux(self,freq):
        ...
        for (i,n) in enumerate(self.note_list):
            ...
            for j in range(len(bf)):
                k[(i*len(bf)+j) % len(k)] = bf[j]
                k[(i*len(bf)+j)%len(k)] = bf[j]
        return k

    def repeat(self,num):
        for i in range(num):
            for j in range(len(self.note_list)):
                self.note_list.append(self.note_list[j])
...
def beat_note(buf,measure,freq): # beat on every 1/4 of a measure
    bf = note(3,2,.5,15000).gen_wave(freq)
    quarter_measure_size = (int)(measure/4)
    for i in range((int)(len(buf)/(measure*4))):
        for j in range(len(bf)):
            buf[i*quarter_measure_size + j] = bf[j]
